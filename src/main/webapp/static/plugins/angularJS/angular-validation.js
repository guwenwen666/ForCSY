...
(function() {
  ...
  function Provider() {
     ...
     * @param message
     * @returns {string}
     * 2017.2.21被重写，支持message的格式化
     */
    this.getErrorHTML = function(message,elemetn,attrs) {
    ...
    var invalidFunc = function(element, validMessage, validation, scope, ctrl, attrs) {
      ...
      if (element.attr('no-validation-message')) {
        messageElem.css('display', 'none');
    	   ...
    	   * 2017-02-21
    	   * */
    	  var formatterMessage = function(message, element, attrs){
    	      var index = attrs.validator.indexOf('=');
    	      if(index > -1){
    	    	  var value = attrs.validator.substring(index+1).replace(/-/g, ',');
    	    	  var val = eval(value);
    	    	  //数值存在的情况
    	    	  if(!!val){
    	        	  ...
    	        	  var reg=/{\d}/g;
    	    		  var i=0;
    	    		  message = message.replace(reg,function(){
    	    		  	i++;
    	    		  	return ''+val[i-1];
    	    	  	  });
    	    	  }
    	  ...
    	  messageToShow = formatterMessage(messageToShow, element, attrs);
    	  /****modify End******/
